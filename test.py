...
# vpp = CDLL(os.path.join(me, 'vpp.so'))
vpp = find_library('vpp')
...
def raw_write_frame(file, array):
    w = array.shape[0]
    h = len(array.shape) <= 1 and 1 or array.shape[1]
...
class InputStream:
    def __init__(self, file, w, h, d):
        ...
        self.h = h
        self.d = d
        self.array = np.ascontiguousarray(np.empty((w, h, d)), dtype='float32')
...
class OutputStream:
    ...
    def write(self, array):
        ...
        assert(array.shape[0] == self.w)
        assert(array.shape[1] == self.h)
        d = len(array.shape) == 2 and 1 or array.shape[2]
        assert(d == self.d)
...
if __name__ == '__main__':
    ...
    input = init_input('bla')
    print(input.w, input.h, input.d)

    output = init_output('blo', input.w, input.h, 1)

    frame = input.read()
    print(frame.mean())

    output.write(frame.mean(2))
